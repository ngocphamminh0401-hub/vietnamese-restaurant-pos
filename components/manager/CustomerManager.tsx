...
export const CustomerManager: React.FC = () => {
    ...
    const handleSaveCustomer = async (customerData: Omit<Customer, 'id' | 'code'>) => {
        if (editingCustomer) {
            ...
            <div className="flex flex-1 overflow-hidden">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            <label className="flex items-center cursor-pointer">
                                <input type="radio" name="cFilter" className="mr-2 text-blue-600 focus:ring-blue-500" checked={typeFilter === 'ALL'} onChange={() => setTypeFilter('ALL')} />
                                <span className="text-sm text-gray-700">Tất cả</span>
                            </label>
                            <label className="flex items-center cursor-pointer">
                                <input type="radio" name="cFilter" className="mr-2 text-blue-600 focus:ring-blue-500" checked={typeFilter === 'PERSONAL'} onChange={() => setTypeFilter('PERSONAL')} />
                                <span className="text-sm text-gray-700">Cá nhân</span>
                            </label>
                            <label className="flex items-center cursor-pointer">
                                <input type="radio" name="cFilter" className="mr-2 text-blue-600 focus:ring-blue-500" checked={typeFilter === 'COMPANY'} onChange={() => setTypeFilter('COMPANY')} />
                ...
                <div className="flex-1 flex flex-col bg-white">
                     <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-white shrink-0">
                        <h3 className="font-bold text-xl text-gray-800">Danh sách Khách hàng</h3>
                        ...
                        <div className="border border-gray-200 rounded-lg overflow-hidden">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-blue-50 text-gray-700 font-bold border-b border-blue-100">
                                    <tr>
                                        <th className="p-4 border-b">Mã khách hàng</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {filteredCustomers.map(c => (
                                        <tr key={c.id} className="hover:bg-gray-50 transition group">
                                            <td className="p-4 font-medium text-blue-600 cursor-pointer">{c.code}</td>
                                            <td className="p-4 font-bold text-gray-800">{c.name}</td>
                                            <td className="p-4 text-gray-600">
                                                <div className="flex items-center"><Phone size={14} className="mr-1.5 text-gray-400"/> {c.phone}</div>
                                            </td>
                                            <td className="p-4 text-gray-600">
                                                {c.type === 'PERSONAL' ? (
                                            ...
                                            <td className="p-4 text-gray-600">
                                                 <div className="flex items-center"><MapPin size={14} className="mr-1.5 text-gray-400"/> {c.address || '-'}</div>
                                            </td>
                                            <td className="p-4">
                                    ...
                                    {filteredCustomers.length === 0 && (
                                         <tr><td colSpan={6} className="p-10 text-center text-gray-400">Không tìm thấy khách hàng nào</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
