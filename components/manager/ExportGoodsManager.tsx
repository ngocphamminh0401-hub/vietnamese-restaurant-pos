...
export const ExportGoodsManager: React.FC = () => {
    ...
    const exportList = inventoryTransactions.filter((t: any) => t.type === 'EXPORT');
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col bg-white">
                     <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-white shrink-0">
                        <h3 className="font-bold text-xl text-gray-800">Danh sách Phiếu xuất hàng</h3>
                        <div className="flex gap-2">
                            <button 
                                onClick={() => setShowAddModal(true)}
                                className="bg-orange-600 hover:bg-orange-700 text-white px-3 py-2 rounded text-sm font-bold flex items-center shadow-sm transition"
                            >
                                <Plus size={16} className="mr-1"/> Tạo phiếu xuất
                            </button>
                        </div>
                   </div>
                    ...
                    <div className="flex-1 overflow-auto p-6">
                        <div className="border border-gray-200 rounded-lg overflow-hidden">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-orange-50 text-gray-700 font-bold border-b border-orange-100">
                                    <tr>
                                        <th className="p-4 border-b">Mã phiếu</th>
                                        <th className="p-4 border-b">Ngày xuất</th>
                                        <th className="p-4 border-b">Lý do xuất</th>
                                        <th className="p-4 border-b text-right">Tổng giá trị</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {exportList.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50 transition">
                                            <td className="p-4 font-medium text-orange-600">{item.code}</td>
                                            <td className="p-4 text-gray-600">{item.createdAt ? new Date(item.createdAt).toLocaleDateString('vi-VN') : '-'}</td>
                                            <td className="p-4 text-gray-800 font-medium">{item.reason}</td>
                                            ...
                                            <td className="p-4 text-center">
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 hover:bg-orange-200 text-orange-700 rounded-lg transition font-medium text-xs"
                                                >
                                    ...
                                    {exportList.length === 0 && (
                                         <tr><td colSpan={5} className="p-12 text-center text-gray-400">Chưa có phiếu xuất hàng nào</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
