...
import { X, Plus, Trash2, Save, PackageOpen } from 'lucide-react';
    ...
    const handleSubmit = async () => {
        ...
        try {
            await addInventoryTransaction({
                type: 'EXPORT',
                reason,
                items: items.map(({ itemId, itemName, quantity, price }) => ({ itemId, itemName, quantity, price })),
                totalAmount,
                note,
                createdBy: 'Admin'
            });
            ...
            alert(error.message || 'Có lỗi xảy ra khi tạo phiếu xuất');
        } finally {
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-red-50">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-lg text-gray-800">Phiếu xuất hàng</h3>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                        <label className="block text-sm font-bold text-gray-700 mb-2">Lý do xuất kho *</label>
                        <select 
                    ...
                    {/* Items Table */}
                    <div>
                        <h4 className="font-bold text-gray-700 mb-3">Danh sách hàng hóa xuất</h4>
                        {items.length === 0 ? (
                            <div className="text-center py-8 text-gray-400 bg-gray-50 rounded-lg border-2 border-dashed">
                                <PackageOpen size={48} className="mx-auto mb-2 opacity-30"/>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
                    ...
                    <div className="flex gap-3">
                        ...
                        <button 
                            ...
                            className="px-6 py-2 bg-red-600 text-white font-bold rounded-lg hover:bg-red-700 flex items-center gap-2 disabled:opacity-50"
                            disabled={isSubmitting || items.length === 0}
                        >
                            <Save size={16}/> {isSubmitting ? 'Đang lưu...' : 'Lưu phiếu xuất'}
                        </button>
                    </div>
                </div>
            </div>
